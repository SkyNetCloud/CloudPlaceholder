...
public class PartyIvAttackParser extends PartyParser {

    private final BattleStatsType stat;

    public PartyIvAttackParser() {
        super(0);
        this.stat = BattleStatsType.NONE;

    }

    protected PartyIvAttackParser(final int slot, final BattleStatsType stat) {
        super(slot);
        this.stat = stat;
    }
    ...
    @Override
    public String getID() {
        return "party_" + this.slot + "_iv_atk";
    }

    @Override
    public void register() {
        for (int i = 1; i <= 6; ++i) {
                SkyExpansion.registerParser(new PartyIvAttackParser(i, stat));
        }
    }
